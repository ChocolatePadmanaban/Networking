...
def server(port):
    ...
    while True:
        ...
        print('the client at {} says {!r}'.format(
            address, text
        ))
...
def client(port):
    ...
    print('The OS assigned me the address {}'.format(
        sock.getsockname()
    ))
    data, address = sock.recvfrom(MAX_BYTE)
    text = data.decode('ascii')
    print('The server {} replied {!r}'.format(
        address,text
    ))
...
type= int, default = 1060,help = 
'UDP port (default 1060)')
